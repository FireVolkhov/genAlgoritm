	...
	createIterator: (size) => {
		...
		return {
			...
			next: function() {
				...
				const result = tableResult.slice(position, end);

				this.position++;
				this.isFinished = (tableResult.length < (end + 1));
				// console.log(this.position, this.isFinished, end, tableResult.length);
				return result;
			}
		};
	}
